...
Once you have the local code set up, install the dependencies with `npm ci`. Then, copy the `sample.env` file to a `.env` file. Configure the two values as needed:

- `NODE_ENV`: Setting this to `production` will enable the `https` server, as well as the default `80` and `443` port listeners. Doing this in a local instance can create issues (low ports are often restricted on your local machine, and https protocol requires an SSL certificate).
- `BASE_PATH`: This is the base path for the CDN. The default value of `http://localhost:8080/` will work in your development instance, but this will need to be updated if you are deploying to production.

At this point, you can make any changes you'd like, then build the TypeScript into JavaScript with `npm run build`. Use `npm start` to spin up the servers and view your changes.

Once you are happy with your changes, run `npm run lint` and `npm run test` to confirm the linter and test suite still pass.
